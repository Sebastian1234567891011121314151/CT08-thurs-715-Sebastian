...
let tileMap2; // NOTE: declared but never loaded in preload() — add a loadStrings() line if you use level 2
...
function setup() {
  ...
  finish.color = "red";
  finish.visible = true; // was the string "true" instead of the boolean true
  ...
  if (startGame) {
    ...
    for (let orb of orbs) {
      if (player.colliding(orb)) {
        ...
        orb.collider = "none";
        player.vel.y = -5;
        playerJump = 0; // let the player jump again after grabbing an orb
      }
    }
  }
}
